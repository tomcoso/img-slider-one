const module = (function () {
    const create = function (imagArray){
        ...
        indexBtns.childNodes[0].classList.add('current')
        ...
        const indexBtnsArray = [...indexBtns.childNodes];

        const getCurrentIndex = function () {
            return imageArray.findIndex(x => x === main.attributes.style.value.match(/\.\S*(?=')/)[0])
        }
        ...
        const advanceSlider = function (optIndex) {
            ...
            (optIndex || optIndex === 0)
            ? nextPic = optIndex
            : nextPic = (currentIndex !== imageArray.length-1) ? (currentIndex + 1) : 0;
        ...
        const reverseSlider = function (optIndex) {
            ...
            (optIndex || optIndex === 0)
            ? nextPic = optIndex
            : nextPic = (currentIndex !== 0) ? (currentIndex - 1) : imageArray.length-1;
        ...
        const gotoSlide = function (event) {
            const target = indexBtnsArray.indexOf(event.path[0]);
            console.log(target)
            const currentIndex = getCurrentIndex();
