...
var Slider;
(function (Slider) {
    Slider.create = function (imagArray, parentSelector) {
        ...
        prevBtnImg.setAttribute("src", "./node_modules/@tomcoso/img-slider-ts/assets/navigate_before_FILL0_wght400_GRAD0_opsz48.png");
        ...
        nextBtnImg.setAttribute("src", "./node_modules/@tomcoso/img-slider-ts/assets/navigate_next_FILL0_wght400_GRAD0_opsz48.png");
        ...
        const getCurrentIndex = function () {
            ...
            const url = matchArray[0];
            return imageArray.findIndex((x) => x === url);
        };
        const advanceSlider = function (optIndex) {
            ...
            next.setAttribute("style", `background-image: url('${imageArray[nextPic]}')`);
            next.animate([
                { transform: "translateX(0%)", opacity: 1 },
                { transform: "translateX(-100%)", opacity: 1, offset: 1 },
                { opacity: 0 },
            ], 500);
            main.setAttribute("style", `background-image: url('${imageArray[nextPic]}')`);
            currentTimeout = setTimeout(advanceSlider, 5000);
        };
        const reverseSlider = function (optIndex) {
            clearTimeout(currentTimeout);
            if (imageArray.length < 2)
                return;
            const currentIndex = getCurrentIndex();
            ...
            indexBtnsArray.forEach((each) => each.classList.remove("current"));
            indexBtnsArray[nextPic].classList.add("current");
            prev.setAttribute("style", `background-image: url('${imageArray[nextPic]}')`);
            prev.animate([
                { transform: "translateX(0%)", opacity: 1 },
                { transform: "translateX(100%)", opacity: 1, offset: 1 },
                { opacity: 0 },
